 ...
 * @returns
 */
export function toEncoding(encodingOrOptions) {
 ...
 */
export function encode(datum) {
  let encoding = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'utf8';
  if (encoding === 'base64') return datum;
  let buffer;
  if (encoding === 'utf8') buffer = Buffer.from(datum);else if (encoding === 'ascii') buffer = Buffer.from(datum, 'latin1');else throw Error(`Invalid encoding type "${encoding}"`);
 ...
 */
export async function readFileGeneric(filepath, encodingOrOptions, command) {
